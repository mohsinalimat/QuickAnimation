...
// 更新displayLink的pause状态，如果动画列表为空并且观察对象为空，停止displayLink刷新
static void updateDisplayLink(POPAnimator *self)
{
    //如果没有观察者并且list为空，或者不允许使用（_disableDisplaylink属性为空）,那么paused = YES
  BOOL paused = (0 == self->_observers.count && self->_list.empty()) || self->_disableDisplayLink;

#if TARGET_OS_IPHONE
    //如果paused与displayLink中的paused属性不同，则同步设置，并打印日志，如果设置为yes开始调用render方法
...
/**
 ...
 @param self Animator
 @param obj 获取dict中obj对应的动画集合
 @param key 删除obj对应动画集合中，为key的动画
 @param cleanup 如果clean为YES,并且动画集合为空，那么会删除整个obj对应的集合字典，默认为yes
 @return 返回删除的元素
 */
...
#if TARGET_OS_IPHONE
    //如果是手机
    //创建displaylink,根据屏幕刷新频率进行回调，每次刷新调用render方法
  _displayLink = [CADisplayLink displayLinkWithTarget:self selector:@selector(render)];
    //暂停调用
  _displayLink.paused = YES;
    //加入commonRunloop，不受trackingMode影响
...
- (void)_processPendingList
{
  ...
  CFTimeInterval time = [self _currentRenderTime];
    //将_pendingList中还没进行的动画加入list中，然后清理_pendingList,并删除runloop 的观察者
...
//创建runloop观察者，让观察者在线程即将休眠或者线程退出的时候，调用processPendingList函数
...
{
    //如果动画对象anim，和动画作用对象obj都为空，直接返回
  if (!anim || !obj) {
    return;
  }

    //如果没有给出key（key = nil) ,那么生成一个uuid作为key
  if (!key) {
  ...
  if (nil == keyAnimationDict) {
    ...
    CFDictionarySetValue(_dict, (__bridge void *)obj, (__bridge void *)keyAnimationDict);
      ...
      //检查动画集合中是否已经存在键值为key的动画
    POPAnimation *existingAnim = keyAnimationDict[key];
      
    if (existingAnim) {
      //如果存在，则解锁
      pthread_mutex_unlock(&_lock);
        //如果存在的动画和需要添加的相同，则直接返回
      if (existingAnim == anim) {
        return;
      }
        //如果不相同，则把原先的动画删除了（后面将会加入新的动画）
  ...
  //加入了list和pendinglist中
  _list.push_back(item);
  _pendingList.push_back(item);

  // 支持动画的复用，所以使用前应该重置动画对象中的状态和属性
  POPAnimationGetState(anim)->reset(true);

  // 更新displayLink的pause状态
  updateDisplayLink(self);
...
/**
 刷新频率，如果是真实设备，返回的是屏幕的刷新率，否则返回的是timer的频率
...
{
    //如果是真实设备，直接返回内核时间，如果是模拟器，需要根据慢放系数对时间进行修正
  CFTimeInterval time = CACurrentMediaTime();

#if TARGET_IPHONE_SIMULATOR
    //模拟器支持slow-motion动画慢放，这需要计算渲染哪个时间的动画，
    //_slowMotionAccumulator为慢放后再恢复正常，产生的时间偏移量。
  time += _slowMotionAccumulator;
    //拖拽系数，如果f为10，说明动画慢10倍
